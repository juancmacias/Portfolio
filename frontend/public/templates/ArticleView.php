...
// Función para renderizar Markdown a HTML (simplificado y seguro)
function parseMarkdown($text) {
    if (empty($text)) {
        return '<p class="text-muted">Sin contenido</p>';
    }
    
    // Debug: Log primeros 100 caracteres
    if (isset($_SERVER['HTTP_HOST']) && strpos($_SERVER['HTTP_HOST'], 'localhost') !== false) {
        error_log("parseMarkdown - Primeros 100 chars: " . substr($text, 0, 100));
    }
    
    // Proteger el texto para evitar problemas con regex
    $text = trim($text);
    
    // NORMALIZAR: Convertir cuádruple backtick a triple backtick (estándar Markdown)
    $text = preg_replace('/````/', '```', $text);
    
    // 1. Convertir bloques de código con triple backtick PRIMERO
    // Soportar con o sin saltos de línea al inicio/final
    $text = preg_replace_callback('/```(\w*)\s*(.*?)\s*```/s', function($matches) {
        $language = !empty($matches[1]) ? htmlspecialchars($matches[1], ENT_QUOTES, 'UTF-8') : 'plaintext';
        $code = htmlspecialchars(trim($matches[2]), ENT_QUOTES, 'UTF-8');
        return "\n\n" . '<pre class="bg-dark text-light p-3 rounded overflow-auto my-3"><code class="language-' . $language . '">' . $code . '</code></pre>' . "\n\n";
    }, $text);
    
    // 2. Convertir código inline (solo backticks únicos, no dobles/triples)
    $text = preg_replace('/(?<!`)(?<!`)(`[^`\n]+`)(?!`)(?!`)/', '<code class="bg-light px-2 py-1 rounded text-danger">$1</code>', $text);
    $text = str_replace(['<code class="bg-light px-2 py-1 rounded text-danger">`', '`</code>'], ['<code class="bg-light px-2 py-1 rounded text-danger">', '</code>'], $text);
    
    // 3. Convertir encabezados
    $text = preg_replace('/^### (.+)$/m', '<h4 class="mt-3 mb-2">$1</h4>', $text);
    $text = preg_replace('/^## (.+)$/m', '<h3 class="mt-4 mb-3">$1</h3>', $text);
    $text = preg_replace('/^# (.+)$/m', '<h2 class="mt-4 mb-3">$1</h2>', $text);
    
    // 4. Convertir negritas y cursivas
    $text = preg_replace('/\*\*\*(.+?)\*\*\*/', '<strong><em>$1</em></strong>', $text);
    $text = preg_replace('/\*\*(.+?)\*\*/', '<strong>$1</strong>', $text);
    $text = preg_replace('/\*(.+?)\*/', '<em>$1</em>', $text);
    
    // 5. Convertir enlaces
    $text = preg_replace('/\[([^\]]+)\]\(([^\)]+)\)/', '<a href="$2" class="text-primary" target="_blank" rel="noopener noreferrer">$1</a>', $text);
    
    // 6. Convertir saltos de línea a párrafos
    $text = nl2br($text);
    
    // 7. Limpiar saltos de línea múltiples
...
function renderArticleView($article) {
    // Sanitizar datos EXCEPTO el contenido que será procesado por parseMarkdown
    $title = htmlspecialchars($article['title'] ?? '', ENT_QUOTES, 'UTF-8');
    $excerpt = htmlspecialchars($article['excerpt'] ?? '', ENT_QUOTES, 'UTF-8');
    $content = $article['content'] ?? ''; // NO escapar aquí, parseMarkdown lo hará
            ...
            <div class="row">
                <div class="col-lg-8 mx-auto">
                    ...
                    <div class="article-content">
                        <?php echo parseMarkdown($content); ?>
                    </div>
