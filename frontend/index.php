<?php
/**
 * Universal SSR Entry Point
 * - Primera carga → PHP genera HTML completo con datos (SSR)
 * - React hidrata → Toma control del DOM sin re-renderizar
 * - Navegación posterior → SPA fluido sin recargas
 *
 * Ventajas:
 *   ✓ SEO perfecto (Googlebot y usuarios ven mismo HTML)
 *   ✓ Primera carga instantánea (HTML ya renderizado)
 *   ✓ Funciona sin JavaScript (accesibilidad)
 *   ✓ Experiencia SPA después de hidratar
 *
...
try {
    ...
    header('Content-Type: text/html; charset=UTF-8');
    header('X-Powered-By: PHP-SSR-Universal');
    header('X-SSR-Route: ' . $route);
    header('X-Hydration: React');
